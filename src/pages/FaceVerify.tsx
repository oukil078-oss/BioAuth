...
export default function FaceVerify() {
  ...
  const { user } = (location.state || {}) as { user?: { id: string; username: string; face_enrolled: boolean; created_at: string; last_login_at: string | null } };
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      if (currentCount === 0) { setError('No face detected. Please position your face in the frame.'); setStatus('ready'); return; }
      ...
      if (response.verified) {
        ...
        setStatus('success');
        setTimeout(() => { navigate('/profile', { state: { user: response.user, confidence: response.confidence } }); }, 2000);
      } else {
        ...
        <div className="w-full max-w-lg">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-slate-500 mt-2">Verify your identity to complete sign-in, {user.username}</p>
            </div>
            ...
            {status === 'denied' && (
              <div className="text-center py-8">
                ...
                <div className="flex items-center justify-center gap-3">
                  <button onClick={() => window.location.reload()} className="inline-flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-700 transition-colors">
                    <RefreshCw className="w-4 h-4" /> Try Again
                  </button>
                  <button onClick={() => navigate('/signin')} className="inline-flex items-center gap-2 border border-slate-300 text-slate-700 px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors">
                    Back to Sign In
                  </button>
                </div>
              </div>
            ...
            {!isTerminal && (
              <>
                ...
                <div className="relative rounded-xl overflow-hidden bg-slate-900 mb-4 aspect-[4/3]">
                  ...
                  {!cameraReady && (
                    <div className="absolute inset-0 bg-slate-900 flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="text-xs text-slate-500 text-center max-w-[200px]">{status === 'loading_models' ? 'Preparing facial recognition engine' : 'Please allow camera permission'}</p>
                    </div>
                  )}
                  {cameraReady && (
                    <>
                ...
                <p className="text-xs text-slate-500 text-center mb-4">Look directly at the camera. Match the angle used during enrollment.</p>
                {status === 'error' ? (
                  ...
                  <button onClick={handleVerify} disabled={status !== 'ready' || faceCount !== 1} className="w-full bg-teal-600 text-white py-3 rounded-xl font-medium hover:bg-teal-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
                    <ShieldCheck className="w-4 h-4" /> Verify Face
                  </button>
                )}
              </>
            )}
