...
async function syncVerifyToNotion(
  notionToken: string,
  username: string,
  userId: string,
  success: boolean,
  ...
  if (failureReason) {
    properties["Failure Reason"] = { rich_text: [{ text: { content: failureReason } }] };
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile) {
      return new Response(
        JSON.stringify({ error: "No face enrollment found for this user" }),
    ...
    if (match) {
      ...
      if (notionToken && user) {
        syncVerifyToNotion(notionToken, user.username, user_id, true, confidence).catch(() => {});
      }

      return new Response(
        JSON.stringify({ success: true, verified: true, confidence, user }),
      ...
      if (notionToken && user) {
        syncVerifyToNotion(
          notionToken, user.username, user_id, false, confidence, "Face does not match"
        ).catch(() => {});
      }

      return new Response(
        JSON.stringify({ success: false, verified: false, confidence, error: "Face does not match. Access denied." }),
