...
async function syncEnrollmentToNotion(notionToken: string, userId: string, username: string, isReEnrollment: boolean): Promise<void> {
  ...
  if (userPageId) {
    await fetch(`https://api.notion.com/v1/pages/${userPageId}`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${notionToken}`, "Content-Type": "application/json", "Notion-Version": "2022-06-28" },
      body: JSON.stringify({ properties: { "Face Enrolled": { checkbox: true }, Status: { select: { name: "Active" } } } }),
    });
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: user } = await supabase.from("users").select("id, username, face_enrolled").eq("id", user_id).maybeSingle();
    ...
    const { error: upsertError } = await supabase.from("biometric_profiles").upsert({ user_id, face_descriptor, enrolled_at: new Date().toISOString() }, { onConflict: "user_id" });
    if (upsertError) return new Response(JSON.stringify({ error: "Failed to store face data" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    await supabase.from("users").update({ face_enrolled: true }).eq("id", user_id);

    const notionToken = Deno.env.get("NOTION_TOKEN");
    if (notionToken) syncEnrollmentToNotion(notionToken, user_id, user.username, isReEnrollment).catch(() => {});
